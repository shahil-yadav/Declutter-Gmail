...
func ListUserById(userId int) (User, error) {
	...
	row := DB.QueryRow("SELECT email, full_name, cover_photo, user_id from users WHERE user_id=?", userId)
	err := row.Scan(&user.Email, &user.FullName, &user.CoverPhoto, &user.UserId)
	if err != nil {
		return User{}, err
	}
